#!/bin/bash
# sunset-cron.sh
# Fetches today's sunset time and sets a cron job X minutes before sunset.
...
logger "setsunset.sh cronjob set for $(date -d "$START_TIME minutes ago $SUNSET_LOCAL" +"%Y-%m-%d %H:%M:%S")"
